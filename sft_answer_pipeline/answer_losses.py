...
def get_answer_loss_fn(noise, graph, train, sampling_eps=1e-3):
    def loss_fn(model, input_ids, train_mask, t=None):
        if t is None:
        ...
        perturbed = torch.where(train_mask, sampled, input_ids)
        ...
        token_loss = token_loss * train_mask.to(token_loss.dtype)
        denom = train_mask.sum(dim=-1).clamp_min(1).to(token_loss.dtype)
...
def evaluate_answer_loss(model, ema, noise, graph, loader, device, eval_batches=0):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            loss = loss_fn(model, input_ids, train_mask).mean()

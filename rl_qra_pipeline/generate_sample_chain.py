...
def csv_visible_cell(value: Any) -> str:
    """Return an Excel/VSCode-safe one-line CSV cell string.

    Keep the visual mask square (□), but render real control characters as
    literal backslash escapes.  This prevents generated tokens such as a real
    newline from splitting one logical CSV row into multiple physical lines.
    """
    if value is None:
        return ""
    text = str(value)
...
def write_csv(path: Path, rows: List[Dict[str, Any]], fields: Sequence[str] = CHAIN_FIELDS) -> None:
    ...
    with open(path, "w", encoding="utf-8-sig", newline="") as f:
        ...
        writer.writeheader()
        for row in rows:
            writer.writerow({k: csv_visible_cell(row.get(k, "")) for k in fields})
